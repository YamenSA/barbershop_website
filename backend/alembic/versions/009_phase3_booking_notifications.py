...
(a) Add cancellation_token (unique) and origin (enum, default walk_in)
    to appointments.
(b) Create notification_logs table with FK + index on appointment_id.
(c) D1-Fix: tstzrange constraint already corrected in migration 007 —
    no action needed.
(d) A1-Idempotenz: partial unique index uq_reminder_sent on
    notification_logs.
"""
...
def upgrade() -> None:
    ...
    appointment_origin = sa.Enum(
        "online", "walk_in", name="appointmentorigin"
    )
    appointment_origin.create(op.get_bind(), checkfirst=True)

    op.add_column(
        "appointments",
        sa.Column("cancellation_token", sa.String(), nullable=True),
    )
    op.create_unique_constraint(
        "uq_appointments_cancellation_token",
        "appointments",
        ["cancellation_token"],
    )
    op.add_column(
    ...
    # (d) Partial unique index — prevents duplicate reminders
    op.create_index(
        "uq_reminder_sent",
        "notification_logs",
        ["appointment_id"],
